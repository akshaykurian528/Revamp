document.addEventListener('DOMContentLoaded', () => {
    ...
    const animateSlide = (direction) => {
        ...
        slideImgElem.src = `${imageBasePath}/img${currentImg}.jpg`;
    ...
    // Add click event listeners to the preview slides
    prevSlides.forEach((prev) => {
        prev.addEventListener('click', (event) => {
            event.stopPropagation();

            const clickedPrev = event.currentTarget;
    ...
    document.addEventListener("click", (event) => {
        ...
        if (clickPosition < slideWidth / 2 && currentImg !== 1) {
            currentImg--;
            animateSlide("left");
        } else if (clickPosition > slideWidth / 2 && currentImg !== totalSlides) {
            currentImg++;
            animateSlide("right");
        }
